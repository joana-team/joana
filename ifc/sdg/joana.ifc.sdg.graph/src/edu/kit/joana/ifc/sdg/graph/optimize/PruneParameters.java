...
public class PruneParameters {
  ...
  private void initWorklist (){
    ...
    for (Iterator<SDGNode> i = g.vertexSet().iterator(); i.hasNext(); ) {
      ...
      if (currNode.isParameter() /*&& !(currNode.getKind() == SDGNode.Kind.EXIT)*/){
  ...
  /**
     * Returns <tt>true</tt> if this list of edges contains a
     * edge of kind kind
     *
     * @param outEdges list of edges
     * @param kind type of edge
     * @return <tt>true</tt> if this list of edges contains a
     * edge of specified kind
     */
    private static boolean hasEdge(Collection<SDGEdge> outEdges, SDGEdge.Kind kind){
      for ( SDGEdge e : outEdges ) {
        if ( e.getKind() == kind ) return true;
      }
      return false;
    }
  ...
  /**
   * Returns <tt>true</tt> if this list of edges contains a
   * CE edge from a CALL or ENTRY node.
   *
  ...
  private static boolean isFirstLevelNode(Collection<SDGEdge> inEdges) {
    for ( SDGEdge e : inEdges ) {
      ...
      if ( ( e.getKind() == Kind.CONTROL_DEP_EXPR) &&
           (currNode.getKind() == SDGNode.Kind.CALL ||
            currNode.getKind() == SDGNode.Kind.ENTRY) )
        return true;
    }
    return false;
  }
  ...
  /**
   * adds nodes to worklist
   * if opposite vertex is of expected kind
   * and connecting edge is the expected kind of edge
   *
   * @param inEdges list of edges
   * @param n actual node
   * @param edgeKind specified kind of edge
   */
  ...
  private void addParent(Collection<SDGEdge> inEdges) {
    ...
    for ( SDGEdge e : inEdges ) {
      SDGNode currNode = e.getSource();
      if ( e.getKind() == Kind.CONTROL_DEP_EXPR &&
          currNode.isParameter() /*&& !(currNode.getKind() == SDGNode.Kind.EXIT)*/){
  ...
  private void processIn(SDGNode n, Collection<SDGEdge> inEdges, Collection<SDGEdge> outEdges, SDGEdge.Kind param,
    ...
    boolean ddExists = hasEdge( outEdges, Kind.DATA_DEP ) || hasEdge( outEdges, Kind.DATA_HEAP ) || hasEdge( outEdges, Kind.DATA_ALIAS );
  ...
  private boolean hasNoOther(SDGEdge.Kind param, SDGNode n) {
	for (SDGEdge edge : g.edgesOf(n)) {
	  if (edge.getKind() != param && (edge.getKind() != Kind.DATA_DEP || edge.getKind() != Kind.DATA_HEAP
			  || edge.getKind() != Kind.DATA_ALIAS) &&
  ...
  private void processOut(SDGNode n, Collection<SDGEdge> inEdges, Collection<SDGEdge> outEdges, SDGEdge.Kind param,
  	...
  	boolean ddExists = hasEdge( inEdges, Kind.DATA_DEP ) || hasEdge( inEdges, Kind.DATA_HEAP ) || hasEdge( inEdges, Kind.DATA_ALIAS );
	...
	private void remove(Collection<SDGEdge> inEdges, SDGNode n) {
		...
		addParent(inEdges);
		if (DEBUG)
			System.out.println("remove Node : " + n.toString());
		g.removeVertex(n);
   ...
   *
   * @param g SDG
   * @return graph after slicing algorithm with probably reduced number
   * of nodes
   */
  public void prune () {
    initWorklist();
    ...
    if (DEBUG) {
      System.out.println();
      System.out.println("initial Size of worklist : "+worklist.size());
      ...
      if ( currNode.getKind() == SDGNode.Kind.FORMAL_IN ) {
        ...
        processIn(currNode, inEdges, outEdges, Kind.PARAMETER_OUT, true);
         ...
         // formal_out-node and exit, do not remove the exit node!
      	processOut(currNode, inEdges, outEdges, Kind.PARAMETER_OUT, currNode.getKind() == SDGNode.Kind.FORMAL_OUT);
      }
    }
    if (DEBUG) {
      System.out.println("Nodes: ");
