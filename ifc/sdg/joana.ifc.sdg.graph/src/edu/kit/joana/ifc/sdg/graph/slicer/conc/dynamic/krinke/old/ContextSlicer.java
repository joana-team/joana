...
public class ContextSlicer {
     ...
     */
    public ContextSlicer(SDG g, ContextManager man, MHPAnalysis m){
        sdg = g;
        this.man = man;
    ...
    /**
     * Creates a new worklist element fom the given values.
     * If this element ws not inserted into the worklist yet,
    ...
    protected void addToWorklist(LinkedList<WorklistElement> worklist,
        ...
        for (int i = 0; i < states.size(); i++) {
            try{
            if (!mhp.isParallel(node, thread, i)) {
                newStates.set(i, con);
            }
            } catch(NullPointerException e) {
                System.out.println("Node: "+node);
                for (int t : node.getThreadNumbers()) System.out.println("      "+t);
                System.out.println("Context: "+con);
                System.out.println("States: "+states);
                System.out.println("Thread: "+thread);
                System.out.println("node: ("+node+", "+thread+")");
