...
public class ISCRBuilder {
     ...
     */
    public ISCRGraph[] buildISCRGraphs(CFG icfg) {
        // an array for the data of the ISCR graphs to build, one for every thread instance
        CFG[] icfgs = new CFG[icfg.getNumberOfThreads()];

        // an array for the ISCR graphs
        ...
        // distribute the nodes to the icfgs-array
        for (SDGNode node : icfg.vertexSet()) {
            // add the node to every thread instance it belongs to
            for (int thread : node.getThreadNumbers()) {
                icfgs[thread].addVertex(node);
            }
        }

        // distribute the edges to the icfgs-array
        for (SDGEdge edge : icfg.edgeSet()) {
            ...
            int[] st = edge.getSource().getThreadNumbers();
            int[] tt = edge.getTarget().getThreadNumbers();

            // add the edge to every thread instance it belongs to
            for (int s : st) {
                for (int t : tt) {
                    if (s == t) {
                    // add only if both source and edge are in the graph
                        icfgs[t].addEdge(edge);
                    }
                }
            }
        }
     ...
     */
    private ISCRGraph build(CFG icfg, int thread) {
    	if (DEBUG) System.out.println("Building ISCR graph for thread "+thread);
...
//    	if (VERIFY) testReducedICFG2(newICFG, reduced);
        ...
        ISCRGraph iscrGraph = new ISCRGraph(reduced, thread, reduced.getRoot(), nodeMap, params);
        if (VERIFY) testISCRGraph(nodeMap, iscrGraph);
        if (VERIFY) testISCRGraphNodes(icfg, iscrGraph);
//        if (VERIFY) testISCRGraphEdges(icfg, iscrGraph);
