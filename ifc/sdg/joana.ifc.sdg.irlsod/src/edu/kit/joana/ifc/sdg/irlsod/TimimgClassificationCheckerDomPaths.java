...
public class TimimgClassificationCheckerDomPaths<L> extends IFC<L>{
	...
	public TimimgClassificationCheckerDomPaths(SDG sdg, IStaticLattice<L> secLattice, Map<SDGNode, L> userAnn, PreciseMHPAnalysis mhp, ICDomOracle cdomOracle, PredecessorMethod predecessorMethod) {
	...
	protected Map<Pair<VirtualNode,VirtualNode>, L> initCLE() {
		Map<Pair<VirtualNode,VirtualNode>, L> ret = new HashMap<>();
		for (SDGNode n : icfg.vertexSet()) {
			for (SDGNode m : icfg.vertexSet()) {
				for (int threadN : n.getThreadNumbers()) {
					for (int threadM : m.getThreadNumbers()) {
						...
						if (mhp.isParallel(vn,vm)) ret.put(Pair.pair(vn,vm), l.getBottom());
					}
				}
			}
		}
