 ...
 */
public class SimpleLeqPropagator<T> extends UnaryOperator<SecLevelVariable<T>> {

	private IStaticLattice<T> secLattice;

	public SimpleLeqPropagator(IStaticLattice<T> secLattice) {
		this.secLattice = secLattice;
	}

	@Override
	public byte evaluate(SecLevelVariable<T> lhs, SecLevelVariable<T> rhs) {
		boolean changed = lhs.meet(rhs.getSecLevel(), secLattice);
		if (changed) {
