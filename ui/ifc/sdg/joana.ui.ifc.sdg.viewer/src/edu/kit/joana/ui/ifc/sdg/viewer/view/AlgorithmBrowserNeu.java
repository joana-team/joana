...
public class AlgorithmBrowserNeu extends SelectionDialog {
	...
	public AlgorithmBrowserNeu(Shell parentShell) {
		super(parentShell);
	...
	protected Control createDialogArea(Composite container) {
		Composite dialog = new Composite(container, SWT.NULL);
		FormLayout thisLayout = new FormLayout();
		dialog.setLayout(thisLayout);
		dialog.setSize(535, 553);
		{
			...
			composite1 = new Composite(dialog, SWT.BORDER);
		...
		{
			// the message field
			text1 = new Text(dialog, SWT.MULTI | SWT.WRAP | SWT.BORDER);
	...
	protected void okPressed() {
		...
		if (!(obj instanceof TreeParent)) {
			...
			ArrayList<Algorithm> results = new ArrayList<Algorithm>();
			results.add(alg);
			setResult(results);
		}
	    super.okPressed();
