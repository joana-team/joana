...
public class SimpleStaticInitializers {
	...
	private void addFakeCall(final PDG callee) {
		...
		lastCall = fakeRoot.createNode(label, Kind.CALL, type, PDGNode.DEFAULT_NO_LOCAL, PDGNode.DEFAULT_NO_LOCAL);
		...
		for (int i = 0; i < in.length; i++) {
			...
			PDGNode actIn = fakeRoot.createNode("param " + (i + 1) + " [?]", PDGNode.Kind.ACTUAL_IN, pType, PDGNode.DEFAULT_NO_LOCAL, PDGNode.DEFAULT_NO_LOCAL);
			in[i] = actIn;
		}
		...
		if (tgt.getReturnType() != TypeReference.Void) {
			retVal = fakeRoot.createNode("ret 0", PDGNode.Kind.ACTUAL_OUT, type, PDGNode.DEFAULT_NO_LOCAL, PDGNode.DEFAULT_NO_LOCAL);
		}
		final PDGNode excVal = fakeRoot.createNode("ret _exception_", PDGNode.Kind.ACTUAL_OUT,
				TypeReference.JavaLangException, PDGNode.DEFAULT_NO_LOCAL, PDGNode.DEFAULT_NO_LOCAL);
