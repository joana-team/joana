...
public class MaximalPathPostDominators<V  extends IntegerIdentifiable, E extends KnowsVertices<V>> {
	...
	private MaximalPathPostDominators(DirectedGraph<V, E> graph) {
		this.graph = graph;
		final ISinkdomEdge<Node<V>> dummy = new ISinkdomEdge<MaximalPathPostDominators.Node<V>>(null, null);
		...
		this.workset = new TreeSet<>(new Comparator<Node<V>>() {
			public int compare(MaximalPathPostDominators.Node<V> o1, MaximalPathPostDominators.Node<V> o2) {
	...
	void newEdge(Node<V> x, Node<V> z) {
		...
		if (z != null) result.addEdgeUnsafe(x, z, new ISinkdomEdge<MaximalPathPostDominators.Node<V>>(x, z));
		x.next = z;
	}
	...
	public static <V extends IntegerIdentifiable, E extends KnowsVertices<V>> MaximalPathPostDominators<V, E> compute(DirectedGraph<V, E> graph, Iterable<Set<V>> sccs) {
