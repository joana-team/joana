...
public class ModRefDataFlow {
	...
	private static Map<Node, PDGNode> createPDGNodes(final ModRefControlFlowGraph cfg, final PDG pdg,
		...
		for (final Node n : cfg) {
			...
			case FORMAL_IN: {
				// create form-in
				final PDGNode fIn = createPDGNode(pdg, n.getCandidate(), pdg.entry, PDGNode.Kind.FORMAL_IN);
				map.put(n, fIn);
				pdgnode2modref.put(fIn.getId(), n.getCandidate());
			} break;
			case FORMAL_OUT: {
				// create form-out
				final PDGNode fOut = createPDGNode(pdg, n.getCandidate(), pdg.entry, PDGNode.Kind.FORMAL_OUT);
				map.put(n, fOut);
				pdgnode2modref.put(fOut.getId(), n.getCandidate());
			} break;
			case ACTUAL_IN: {
				// create actual-in
				final PDGNode aIn = createPDGNode(pdg, n.getCandidate(), n.getNode(), PDGNode.Kind.ACTUAL_IN);
				map.put(n, aIn);
				pdgnode2modref.put(aIn.getId(), n.getCandidate());
			} break;
			case ACTUAL_OUT: {
				// create actual-out
				final PDGNode aOut = createPDGNode(pdg, n.getCandidate(), n.getNode(), PDGNode.Kind.ACTUAL_OUT);
				map.put(n, aOut);
	...
	private static PDGNode createPDGNode(final PDG pdg, final ModRefFieldCandidate c, final PDGNode parent,
			final PDGNode.Kind kind) {
		final String label = c.toString();
		final TypeReference type = c.getType();
		final PDGNode newNode = pdg.createNode(label, kind, type);
