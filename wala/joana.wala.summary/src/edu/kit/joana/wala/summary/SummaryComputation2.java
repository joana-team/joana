...
//		relevantEdges.add(SDGEdge.Kind.SYNCHRONIZATION);
    ...
    private Collection<SDGEdge> computeSummaryEdges(Collection<SDGEdge> summary,
    		Set<SDGNode> entries, IProgressMonitor progress) throws CancelException {
    	//HashSet<SDGEdge> actInOutSummaryEdge = new HashSet<SDGEdge>();
    	HashMap<SDGNode, Set<SDGNode>> pathEdge = new HashMap<SDGNode, Set<SDGNode>>();
    	HashMap<SDGNode, Set<SDGNode>> aoPaths = new HashMap<SDGNode, Set<SDGNode>>();
    	final LinkedList<Edge> worklist = new LinkedList<>();
        ...
        for (SDGNode entry : entries) {
        	for (SDGNode n : entry2outs.get(entry)) {
            	...
            	Set<SDGNode> pathSet = new HashSet<>();
            	pathSet.add(n);
                pathEdge.put(n, pathSet);
            ...
            switch(k) {
                case ACTUAL_OUT:
                	if (fullyConnected != null && fullyConnected.contains(next.source.getId())) {
                		propagateAllActIns(worklist, pathEdge, aoPaths, next.source, next.target);
                	} else {
                		Set<SDGEdge> incEdges;
                		//synchronized (graph) {
                			incEdges = graph.incomingEdgesOf(next.source);
                		//}
	                    for (SDGEdge e : incEdges) {
	                        ...
	                        if (kind == sumEdgeKind
	                    		...
	                    		propagate(worklist, pathEdge, aoPaths, new Edge(e.getSource(), next.target));
	                        }
	                    }
                	}
                    break;
                ...
                case FORMAL_IN:
                    ...
                    for (Edge e : aiaoPairs) {
                        ...
                        boolean edgeIsNew;
                        synchronized (graph) {
                    		edgeIsNew = graph.addEdge(e.source, e.target, sum);
                        }
                        if (edgeIsNew) {
                            synchronized (graph) {
                            	summary.add(sum);
                            ...
                            if (s != null) {
                                for (SDGNode t : s) {
                                    propagate(worklist, pathEdge, aoPaths, new Edge(sum.getSource(), t));
                                }
                            }
                        }
                    }
...
//                    }
    ...
    private void propagateAllActIns(LinkedList<Edge> worklist, Map<SDGNode, Set<SDGNode>> pathEdge,
    		Map<SDGNode, Set<SDGNode>> aoPaths, SDGNode outNode, SDGNode target) {
    	for (SDGNode inNode : out2in.get(outNode.getId())) {
    		propagate(worklist, pathEdge, aoPaths, new Edge(inNode, target));
    	}
    }

    private void propagate(LinkedList<Edge> worklist, Map<SDGNode, Set<SDGNode>> pathEdge,
    		Map<SDGNode, Set<SDGNode>> aoPaths, Edge e) {
...
//    	}
//
        Set<SDGNode> pathSet = pathEdge.get(e.target);
        if (pathSet.add(e.source)) {
            worklist.add(e);
        }
        if (e.source.getKind() == SDGNode.Kind.ACTUAL_OUT) {
            Set<SDGNode> s = aoPaths.get(e.source);
            if (s == null) {
